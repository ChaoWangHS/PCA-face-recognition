...
{
    string trainListFilePath = "D:\\pattern\\list\\train_list.txt";
	...
	cout << "读取出来的向量维度：eigenVec: " << eigenVec.rows << "*" << eigenVec.cols << endl;
    ...
    if ( choice == 0 ) {
        ...
        while ( getFrame.getNextFrame(frame) ) {
            ...
            int key = waitKey(3000); //点击脸部图像的窗口（不是cmd的）再任意按键（除了esc键）就可get一张脸部照片 
            if ( key != -1 ) {
                if ( (key & 255) == 27) { // 27 是ESC(escape)的 ASCII 码
                    break;
                }
				else
				{
                    facesCount++;
                    string tempPath = "D:\\pattern\\faces\\temp\\s\\";
        ...
        while (getFrame.getNextFrame(frame)) {
            ...
            if (faceDetector.goodFace()) {
				...
				cout << "eigenVec:" <<eigenVec.size() << endl;
